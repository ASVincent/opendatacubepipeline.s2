...
def _submit_multiple(scattered, env, batch_logdir, batch_outdir, pkgdir,
    ...
    for block in scattered:
        ...
        if test:
            ...
            cli_submit = subprocess.Popen(['qsub', out_fname], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

            if cli_submit.returncode == 0:
                stdout = sli_submit.communicate()[0].decode('ascii').strip()
                if re.match(PBS_JOB_FMT, stdout):
                    submitted_jobs.append(stdout)
...
def main(level1_list, workdir, logdir, pkgdir, env, workers, nodes, memory,
    ...
    return _submit_multiple(scattered, env, batch_logdir, batch_outdir,
