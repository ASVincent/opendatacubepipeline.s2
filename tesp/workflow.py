...
class Package(luigi.Task):
    ...
    def requires(self):
        ...
        self._validate_cfg()
        platform = (
            acquisitions(self.level1, hint=acq_parser_hint)
            .get_all_acquisitions()[0]
            .platform
        )
        ...
        if 'SENTINEL_2' in platform:
            tasks['gqa'] = GQATask(self.level1, self.granule, self.workdir)
    ...
    def run(self):
        inputs = {}
        for _product_name, _product in self.input():
            inputs[_product_name] = _product.path

        package(self.level1, inputs, self.yamls_dir, self.pkgdir, self.granule,
...
class ARDP(luigi.WrapperTask):
    ...
    pkgdir = luigi.Parameter()
    acq_parser_hint = luigi.OptionalParameter(default='')
    s2_aoi = luigi.OptionalParameter(default='')

    def requires(self):
        with open(self.level1_list) as src:
            level1_list = [level1.strip() for level1 in src.readlines()]

        # Tile based areas of interest were introduced with the GQA assessment
        # and are intended to be refactored out
        in_aoi = lambda _: True
        if s2_aoi:
            tile_ids = []
            with open(self.s2_aoi, 'r') as csv:
                tile_ids = ['T' + tile.strip() for tile in csv]

            in_aoi = lambda granule: granule.split('_')[-2] in tile_ids
